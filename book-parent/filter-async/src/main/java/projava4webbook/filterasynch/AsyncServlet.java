...
public class AsyncServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        long timeout = request.getParameter("timeout") == null ? DEFAULT_TIMEOUT : Long.parseLong(request.getParameter("timeout"));

        System.out.println("Entering AsyncServlet.doGet(). Request ID = " + id +
                ", isAsyncStarted = " + request.isAsyncStarted());

        final AsyncContext context;
        if (request.getParameter("unwrap") != null) {
            context = request.startAsync();                     // AsyncContext gets the original request and response
        } else {
            context = request.startAsync(request, response);    // AsyncContext gets the request and response as passed into doGet
        }
        ...
        context.start(thread::doWork);      // The container runs doWork() with its internal thread pool. 
